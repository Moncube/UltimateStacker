...
public abstract class AbstractGuiListEditor extends Gui {
    ...
    public void paint() {
        ...
        setButton(2, GuiUtils.createButtonItem(Material.OAK_FENCE_GATE,
                        TextUtils.formatText("&cBack")),
                (event) -> {
                    guiManager.showGUI(event.player, returnGui);
                    ((GuiLootEditor) returnGui).paint();
                });
        setButton(6, GuiUtils.createButtonItem(Material.OAK_FENCE_GATE,
                        TextUtils.formatText("&cBack")),
                (event) -> {
                    guiManager.showGUI(event.player, returnGui);
                    ((GuiLootEditor) returnGui).paint();
                });
        /*setButton(3, GuiUtils.createButtonItem(Material.ARROW,
        ...
        setItem(4, GuiUtils.createButtonItem(Material.WRITABLE_BOOK,
                TextUtils.formatText("&9Lore:"),
                lore.isEmpty()
                        ? TextUtils.formatText(Collections.singletonList("&cNo lore set..."))
                        : TextUtils.formatText(lore)));

        setButton(5, GuiUtils.createButtonItem(Material.ARROW,
